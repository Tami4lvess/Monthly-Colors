...
(function renderMoonPanel() {
  ...
  const phases = getMoonPhasesForMonth(currentYear, 4); // Abril

  // Atualiza o subtítulo "Abril XXXX"
  const sub = document.querySelector('.moon-panel-sub');
  if (sub) sub.textContent = `Abril ${currentYear}`;
    ...
    <svg class="moon-svg" width="64" height="64" viewBox="0 0 64 64">
      <defs>
        <radialGradient id="ng1auto" cx="40%" cy="35%" r="60%">
          <stop offset="0%" stop-color="#173326"/>
    ...
    <svg class="moon-svg" width="64" height="64" viewBox="0 0 64 64">
      <defs>
        <radialGradient id="qcgauto" cx="60%" cy="35%" r="65%">
          <stop offset="0%" stop-color="#B6E6C5"/>
          <stop offset="50%" stop-color="#5DB37C"/>
          <stop offset="100%" stop-color="#26734A"/>
        </radialGradient>
      ...
      <ellipse cx="32" cy="32" rx="8" ry="28" fill="#07140E"/>
    </svg>`;
    ...
    <svg class="moon-svg" width="64" height="64" viewBox="0 0 64 64">
      <defs>
        <radialGradient id="lcgauto" cx="35%" cy="30%" r="70%">
          <stop offset="0%" stop-color="#D0EED8"/>
          <stop offset="40%" stop-color="#79C291"/>
          <stop offset="100%" stop-color="#318057"/>
        </radialGradient>
      </defs>
      ...
      <circle
        ...
        r="26"
        fill="url(#lcgauto)"
        stroke="rgba(121,194,145,0.40)"
        stroke-width="1"
      />

      <circle cx="24" cy="22" r="5" fill="rgba(34,100,62,0.25)"/>
      <circle cx="38" cy="30" r="3.5" fill="rgba(34,100,62,0.20)"/>
      <circle cx="28" cy="38" r="4" fill="rgba(34,100,62,0.20)"/>
    </svg>`;
  ...
  if (fullMoon) {
    ...
    gcalLinks.forEach(link => {
      ...
      if (onclick.includes('Lua Rosa')) {
        const d = String(fullMoon.day).padStart(2,'0');
        link.setAttribute('onclick',
          `addToCalendar('${d}/04/${currentYear}','Lua Rosa - Lua Cheia de Abril')`);

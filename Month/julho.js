...
(function renderMoonPanel() {
  ...
  const phases = getMoonPhasesForMonth(currentYear, 7); // Julho

  // Atualiza o subtítulo "Julho XXXX"
  const sub = document.querySelector('.moon-panel-sub');
  if (sub) sub.textContent = `Julho ${currentYear}`;
  ...
  if (fullMoon) {
    ...
    gcalLinks.forEach(link => {
      ...
      if (onclick.includes('Lua dos Cervos')) {
        const d = String(fullMoon.day).padStart(2,'0');
        link.setAttribute('onclick',
          `addToCalendar('${d}/07/${currentYear}','Lua dos Cervos - Lua Cheia de Julho')`);

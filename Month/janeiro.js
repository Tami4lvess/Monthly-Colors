...
function getMoonPhasesForMonth(year, month) {
  ...
  // A diferença TT−UTC não costuma alterar o minuto exibido, mas é importante
  // mantê-la para não deslocar fases que ocorram perto da meia-noite.
  function jdeToDate(jde) {
    const t = year - 2000;
    const deltaTSeconds = 62.92 + 0.32217 * t + 0.005589 * t * t;
    const unixMilliseconds =
      (jde - 2440587.5) * 86400000 - deltaTSeconds * 1000;
  ...
  function getBrazilDateParts(date) {
    const values = Object.fromEntries(
      brazilDateFormatter.formatToParts(date)

...
(function renderMoonPanel() {
  ...
  const phases = getMoonPhasesForMonth(currentYear, 10); // Outubro

  // Atualiza o subtítulo "Outubro XXXX"
  const sub = document.querySelector('.moon-panel-sub');
  if (sub) sub.textContent = `Outubro ${currentYear}`;

  // SVG templates para cada fase (reutiliza os SVGs originais)
  // SVG templates para cada fase
function moonSVG(type) {
  if (type === 'nova') return `
    <svg class="moon-svg" width="64" height="64" viewBox="0 0 64 64">
      <defs>
        <radialGradient id="ng1auto" cx="40%" cy="35%" r="60%">
          <stop offset="0%" stop-color="#3D1826"/>
          <stop offset="100%" stop-color="#180910"/>
        </radialGradient>
      </defs>
      ...
      <circle cx="26" cy="26" r="4" fill="rgba(255,180,200,0.06)"/>
      <circle cx="38" cy="36" r="3" fill="rgba(255,180,200,0.04)"/>
    </svg>`;
    ...
    <svg class="moon-svg" width="64" height="64" viewBox="0 0 64 64">
      <defs>
        <radialGradient id="qcgauto" cx="60%" cy="35%" r="65%">
          <stop offset="0%" stop-color="#E7A1B5"/>
          <stop offset="50%" stop-color="#BB5574"/>
          <stop offset="100%" stop-color="#762A45"/>
        </radialGradient>
      ...
      <ellipse cx="32" cy="32" rx="8" ry="28" fill="#180910"/>
    </svg>`;
    ...
    <svg class="moon-svg" width="64" height="64" viewBox="0 0 64 64">
      <defs>
        <radialGradient id="lcgauto" cx="35%" cy="30%" r="70%">
          <stop offset="0%" stop-color="#F2B5C6"/>
          <stop offset="40%" stop-color="#D37590"/>
          <stop offset="100%" stop-color="#913A56"/>
        </radialGradient>
      </defs>
      ...
      <circle
        ...
        r="26"
        fill="url(#lcgauto)"
        stroke="rgba(213,117,144,0.40)"
        stroke-width="1"
      />

      <circle cx="24" cy="22" r="5" fill="rgba(118,42,69,0.28)"/>
      <circle cx="38" cy="30" r="3.5" fill="rgba(118,42,69,0.23)"/>
      <circle cx="28" cy="38" r="4" fill="rgba(118,42,69,0.23)"/>
    </svg>`;
  ...
  if (fullMoon) {
    ...
    gcalLinks.forEach(link => {
      ...
      if (onclick.includes('Lua do Caçador')) {
        const d = String(fullMoon.day).padStart(2,'0');
        link.setAttribute('onclick',
          `addToCalendar('${d}/10/${currentYear}','Lua do Caçador - Lua Cheia de Outubro')`);

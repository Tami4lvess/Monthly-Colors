...
function getMoonPhasesForMonth(year, month) {
  ...
  // Mantém o mesmo padrão visual em todos os meses:
  // Nova → Crescente → Cheia → Minguante.
  const phaseOrder = { nova: 0, crescente: 1, cheia: 2, minguante: 3 };
  result.sort(
    (a, b) => phaseOrder[a.icon] - phaseOrder[b.icon] || a.date - b.date,
  );
  return result;
}
...
(function renderMoonPanel() {
  ...
  const phases = getMoonPhasesForMonth(currentYear, 12); // Dezembro

  // Atualiza o subtítulo "Dezembro XXXX"
  const sub = document.querySelector('.moon-panel-sub');
  if (sub) sub.textContent = `Dezembro ${currentYear}`;
  ...
  function moonSVG(type) {
    if (type === 'nova') return `
      <svg class="moon-svg" width="64" height="64" viewBox="0 0 64 64">
        <defs><radialGradient id="ng1auto" cx="40%" cy="35%" r="60%"><stop offset="0%" stop-color="#6B1F2B"/><stop offset="100%" stop-color="#24090F"/></radialGradient></defs>
        <circle cx="32" cy="32" r="28" fill="url(#ng1auto)" stroke="#9A3341" stroke-width="1.5"/>
    ...
    if (type === 'crescente') return `
      <svg class="moon-svg" width="64" height="64" viewBox="0 0 64 64">
        <defs>
          <radialGradient id="qcgauto" cx="60%" cy="35%" r="65%"><stop offset="0%" stop-color="#FFD2C4"/><stop offset="50%" stop-color="#EE8D72"/><stop offset="100%" stop-color="#B82E3F"/></radialGradient>
          <clipPath id="qc-clipauto"><rect x="32" y="4" width="28" height="56"/></clipPath>
        </defs>
        <circle cx="32" cy="32" r="28" fill="#24090F" stroke="#9A3341" stroke-width="1"/>
        <circle cx="32" cy="32" r="28" fill="url(#qcgauto)" clip-path="url(#qc-clipauto)"/>
        <ellipse cx="32" cy="32" rx="8" ry="28" fill="#24090F"/>
      </svg>`;
    if (type === 'cheia') return `
      <svg class="moon-svg" width="64" height="64" viewBox="0 0 64 64">
        <defs><radialGradient id="lcgauto" cx="35%" cy="30%" r="70%"><stop offset="0%" stop-color="#FFE7D6"/><stop offset="40%" stop-color="#F3A38F"/><stop offset="100%" stop-color="#C94A55"/></radialGradient></defs>
        <circle cx="32" cy="32" r="26" fill="url(#lcgauto)" stroke="rgba(255,194,154,0.45)" stroke-width="1"/>
        <circle cx="24" cy="22" r="5" fill="rgba(139,38,53,0.22)"/>
        <circle cx="38" cy="30" r="3.5" fill="rgba(139,38,53,0.18)"/>
        <circle cx="28" cy="38" r="4" fill="rgba(139,38,53,0.18)"/>
      </svg>`;
    return `
      <svg class="moon-svg" width="64" height="64" viewBox="0 0 64 64">
        <defs>
          <radialGradient id="qmgauto" cx="40%" cy="35%" r="65%"><stop offset="0%" stop-color="#FFD2C4"/><stop offset="50%" stop-color="#EE8D72"/><stop offset="100%" stop-color="#B82E3F"/></radialGradient>
  ...
  if (fullMoon) {
    ...
    gcalLinks.forEach(link => {
      ...
      if (onclick.includes('Lua Fria')) {
        const d = String(fullMoon.day).padStart(2,'0');
        link.setAttribute('onclick',
          `addToCalendar('${d}/12/${currentYear}','Lua Fria - Lua Cheia de Dezembro')`);

...
(function renderMoonPanel() {
  ...
  const phases = getMoonPhasesForMonth(currentYear, 8); // Agosto

  // Atualiza o subtítulo "Agosto XXXX"
  const sub = document.querySelector('.moon-panel-sub');
  if (sub) sub.textContent = `Agosto ${currentYear}`;
  ...
  if (fullMoon) {
    ...
    gcalLinks.forEach(link => {
      ...
      if (onclick.includes('Lua do Esturjão')) {
        const d = String(fullMoon.day).padStart(2,'0');
        link.setAttribute('onclick',
          `addToCalendar('${d}/08/${currentYear}','Lua do Esturjão - Lua Cheia de Agosto')`);

...
(function renderMoonPanel() {
  ...
  const phases = getMoonPhasesForMonth(currentYear, 9); // Setembro

  // Atualiza o subtítulo "Setembro XXXX"
  const sub = document.querySelector('.moon-panel-sub');
  if (sub) sub.textContent = `Setembro ${currentYear}`;
    ...
    <svg class="moon-svg" width="64" height="64" viewBox="0 0 64 64">
      <defs>
        <radialGradient id="ng1auto" cx="40%" cy="35%" r="60%">
          <stop offset="0%" stop-color="#3A3015"/>
          <stop offset="100%" stop-color="#151108"/>
        </radialGradient>
      </defs>
      ...
      <circle cx="26" cy="26" r="4" fill="rgba(255,226,120,0.06)"/>
      <circle cx="38" cy="36" r="3" fill="rgba(255,226,120,0.04)"/>
    </svg>`;
    ...
    <svg class="moon-svg" width="64" height="64" viewBox="0 0 64 64">
      <defs>
        <radialGradient id="qcgauto" cx="60%" cy="35%" r="65%">
          ...
          <stop offset="50%" stop-color="#E0B936"/>
      ...
      <ellipse cx="32" cy="32" rx="8" ry="28" fill="#151108"/>
    </svg>`;
    ...
    <svg class="moon-svg" width="64" height="64" viewBox="0 0 64 64">
      <defs>
        <radialGradient id="lcgauto" cx="35%" cy="30%" r="70%">
          ...
          <stop offset="40%" stop-color="#E5C44F"/>
          <stop offset="100%" stop-color="#A27612"/>
        </radialGradient>
      </defs>
      ...
      <circle
        ...
        r="26"
        fill="url(#lcgauto)"
        stroke="rgba(229,196,79,0.45)"
        stroke-width="1"
      />

      <circle cx="24" cy="22" r="5" fill="rgba(122,90,0,0.25)"/>
      <circle cx="38" cy="30" r="3.5" fill="rgba(122,90,0,0.20)"/>
      <circle cx="28" cy="38" r="4" fill="rgba(122,90,0,0.20)"/>
    </svg>`;
  ...
  if (fullMoon) {
    ...
    gcalLinks.forEach(link => {
      ...
      if (onclick.includes('Lua da Colheita')) {
        const d = String(fullMoon.day).padStart(2,'0');
        link.setAttribute('onclick',
          `addToCalendar('${d}/09/${currentYear}','Lua da Colheita - Lua Cheia de Setembro')`);

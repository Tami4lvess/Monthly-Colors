...
(function renderMoonPanel() {
  ...
  const phases = getMoonPhasesForMonth(currentYear, 11); // Novembro

  // Atualiza o subtítulo "Novembro XXXX"
  const sub = document.querySelector('.moon-panel-sub');
  if (sub) sub.textContent = `Novembro ${currentYear}`;
  ...
  if (fullMoon) {
    ...
    gcalLinks.forEach(link => {
      ...
      if (onclick.includes('Lua do Castor')) {
        const d = String(fullMoon.day).padStart(2,'0');
        link.setAttribute('onclick',
          `addToCalendar('${d}/11/${currentYear}','Lua do Castor - Lua Cheia de Novembro')`);

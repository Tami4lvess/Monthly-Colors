...
(function renderMoonPanel() {
  ...
  const phases = getMoonPhasesForMonth(currentYear, 3); // Março

  // Atualiza o subtítulo "Março XXXX"
  const sub = document.querySelector('.moon-panel-sub');
  if (sub) sub.textContent = `Março ${currentYear}`;
  ...
  if (fullMoon) {
    ...
    gcalLinks.forEach(link => {
      ...
      if (onclick.includes('Lua do Verme')) {
        const d = String(fullMoon.day).padStart(2,'0');
        link.setAttribute('onclick',
          `addToCalendar('${d}/03/${currentYear}','Lua do Verme - Lua Cheia de Março')`);

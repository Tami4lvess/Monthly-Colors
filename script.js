...
for (let x = -8; x <= 8; x++) {
  for (let z = -6; z <= 6; z++) {
    ...
    if (dist < 7.5 + pseudoNoise(x, z) * 1.5) hillData.push({ x, y: 0, z, type: 'grass' });
  }
}
for (let x = -9; x <= 9; x++) {
  for (let z = -7; z <= 7; z++) {
    const dist = Math.sqrt(x * x * 0.4 + z * z * 0.5);
    if (dist < 8.5 + pseudoNoise(x * 0.7, z * 0.7) * 1.2) hillData.push({ x, y: -1, z, type: 'grass' });
  }
}
for (let x = -7; x <= 6; x++) {
  for (let z = -5; z <= 5; z++) {
    const dist = Math.sqrt(x * x * 0.5 + z * z * 0.6);
    if (dist < 6.0 + pseudoNoise(x, z) * 1.2) hillData.push({ x, y: 1, z, type: 'grass' });
  }
}
for (let x = -5; x <= 4; x++) {
  for (let z = -4; z <= 3; z++) {
    const dist = Math.sqrt(x * x * 0.55 + z * z * 0.65);
    if (dist < 4.5 + pseudoNoise(x, z) * 0.9) hillData.push({ x, y: 2, z, type: 'grass' });
  }
}
for (let x = -4; x <= 3; x++) {
  for (let z = -3; z <= 2; z++) {
    const dist = Math.sqrt(x * x * 0.6 + z * z * 0.7);
    if (dist < 3.5 + pseudoNoise(x, z) * 0.7) hillData.push({ x, y: 3, z, type: 'grass' });
  }
}
for (let x = -3; x <= 2; x++) {
  for (let z = -2; z <= 2; z++) {
    const dist = Math.sqrt(x * x * 0.7 + z * z * 0.8);
    if (dist < 2.8 + pseudoNoise(x, z) * 0.5) hillData.push({ x, y: 4, z, type: 'grass' });
  }
}
for (let x = -2; x <= 1; x++) {
  for (let z = -1; z <= 1; z++) {
...
const rockPositions = [
  ...
  { x: -1, y: 6, z: 1 }, { x: 0, y: 6, z: 1 }, { x: -2, y: 6, z: -1 },
  ...
  { x: -3, y: 2, z: 2 }, { x: -3, y: 3, z: 2 },
  { x: 5, y: 1, z: 1 }, { x: 5, y: 1, z: 0 }, { x: -5, y: 1, z: 0 },
  { x: 2, y: 3, z: -2 }, { x: 2, y: 4, z: -2 }, { x: -3, y: 3, z: -1 },
...
function addLeaf(x, y, z) {
  const key = `${x},${y},${z}`;
  if (!leafSet.has(key)) { leafSet.add(key); leafPositions.push({ x, y, z }); }
}

const canopyCenterX = 0, canopyCenterY = 20, canopyCenterZ = 0;
const canopyRadiusH = 6.5, canopyRadiusV = 4.5;
...
for (let x = -8; x <= 8; x++) {
  for (let y = 15; y <= 26; y++) {
    for (let z = -7; z <= 7; z++) {
      ...
      if (dist < canopyRadiusH + edgeNoise && Math.random() > 0.18) addLeaf(x, y, z);
    }
  }
}

const branchTips = [
  { cx: -5, cy: 17, cz: 0, r: 3.5 }, { cx: -5, cy: 17, cz: 1, r: 2.8 },
  { cx: 5, cy: 17, cz: -1, r: 3.5 }, { cx: 4, cy: 18, cz: 0, r: 3.0 },
  { cx: 1, cy: 17, cz: 3, r: 3.2 }, { cx: 1, cy: 17, cz: -3, r: 3.0 },
  { cx: -1, cy: 17, cz: -3, r: 2.8 },
  { cx: 0, cy: 24, cz: 0, r: 3.0 }, { cx: -2, cy: 23, cz: 1, r: 2.5 },
  { cx: 2, cy: 23, cz: -1, r: 2.5 }, { cx: 1, cy: 24, cz: 1, r: 2.0 },
  { cx: -1, cy: 24, cz: -1, r: 2.0 },
  { cx: -7, cy: 18, cz: 0, r: 2.0 }, { cx: 6, cy: 18, cz: 0, r: 2.0 },
  { cx: 0, cy: 18, cz: 5, r: 2.2 }, { cx: 0, cy: 18, cz: -5, r: 2.2 },
  { cx: -3, cy: 15, cz: 2, r: 2.5 }, { cx: 3, cy: 15, cz: -2, r: 2.5 },
  { cx: -2, cy: 15, cz: -3, r: 2.0 }, { cx: 2, cy: 15, cz: 3, r: 2.0 },
];
  ...
  for (let x = Math.floor(tip.cx - tip.r - 1); x <= Math.ceil(tip.cx + tip.r + 1); x++) {
    for (let y = Math.floor(tip.cy - tip.r); y <= Math.ceil(tip.cy + tip.r + 1); y++) {
      for (let z = Math.floor(tip.cz - tip.r - 1); z <= Math.ceil(tip.cz + tip.r + 1); z++) {
        const dx = x - tip.cx, dy = (y - tip.cy) * 1.15, dz = z - tip.cz;
        const dist = Math.sqrt(dx * dx + dy * dy + dz * dz);
...
for (let i = 0; i < 25; i++) {
  addLeaf(Math.round((Math.random() - 0.5) * 14), Math.floor(Math.random() * 3) + 1, Math.round((Math.random() - 0.5) * 10));
}
...
Object.entries(grassTopMap).forEach(([key, topY]) => {
  ...
  if (!blocked && Math.random() < 0.40) {
    ...
    for (let f = 0; f < numFlowers; f++) {
      ...
      createCustomVoxel(c, gx * step + (Math.random() - 0.5) * 0.5, (topY + 1) * step + voxelSize * 0.22, gz * step + (Math.random() - 0.5) * 0.5, 0.35, 0.35, 0.35, 0, 0, 0.7, 0.0, 'flower');
    }
  }
  if (!blocked && Math.random() < 0.30) {
    const c = pickRandom(grassTuftColors);
    createCustomVoxel(c, gx * step + (Math.random() - 0.5) * 0.6, (topY + 1) * step + voxelSize * 0.32, gz * step + (Math.random() - 0.5) * 0.6, 0.25, 0.55, 0.25, (Math.random() - 0.5) * 0.15, (Math.random() - 0.5) * 0.15, 0.9, 0.0, 'grassTuft');
  }
});
...
Object.entries(grassTopMap).forEach(([key, topY]) => {
  ...
  if (gx < -2 && Math.random() < 0.15 && !rockSet.has(key)) {
    ...
    createCustomVoxel(c, gx * step + (Math.random() - 0.5) * 0.3, (topY + 1) * step + voxelSize * 0.15, gz * step + (Math.random() - 0.5) * 0.3, 0.25, 0.22, 0.25, 0, 0, 0.8, 0.0, 'mushroom');
  }
});
  ...
  voxels.forEach(im => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (im.instanceColor) {
        ...
        colors[i * 3] = im.material.color.r; colors[i * 3 + 1] = im.material.color.g; colors[i * 3 + 2] = im.material.color.b;
      }
    }
...
function generateSnowyPineOffsets() {
  ...
  voxels.forEach(im => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      let cr, cg, cb;
      if (im.instanceColor) { im.getColorAt(i, tmpColor); cr = tmpColor.r; cg = tmpColor.g; cb = tmpColor.b; }
      else { cr = im.material.color.r; cg = im.material.color.g; cb = im.material.color.b; }
      ...
      else if (cat === 'underside') { const uc = new THREE.Color(pickRandom(stoneUndersideColors)); cr = uc.r; cg = uc.g; cb = uc.b * 1.05; }
...
function generateCherryBlossomOffsets() {
  ...
  voxels.forEach(im => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      else if (cat === 'flower') { const pc = new THREE.Color(pickRandom(blossomPink)); cr = pc.r; cg = pc.g; cb = pc.b; }
      else if (cat === 'underside') { const uc = new THREE.Color(pickRandom(dirtColors)); cr = uc.r; cg = uc.g; cb = uc.b; }
      positions[i * 3] = nx; positions[i * 3 + 1] = ny; positions[i * 3 + 2] = nz;
      colors[i * 3] = cr; colors[i * 3 + 1] = cg; colors[i * 3 + 2] = cb;
    }
    snap.set(im, { positions, colors });
...
let morphStartTime = 0, morphFrom = 0, morphTo = 0;
let morphBasePositions = null, morphBaseColors = null;

function snapshotCurrentState() {
  const posMap = new Map(), colMap = new Map();
  const tmpColor = new THREE.Color();
  voxels.forEach(im => {
    const data = instanceData.get(im);
    if (!data) return;
    ...
    for (let i = 0; i < count; i++) {
      pos[i * 3] = data.origPositions[i * 3]; pos[i * 3 + 1] = data.origPositions[i * 3 + 1]; pos[i * 3 + 2] = data.origPositions[i * 3 + 2];
      if (im.instanceColor) { im.getColorAt(i, tmpColor); col[i * 3] = tmpColor.r; col[i * 3 + 1] = tmpColor.g; col[i * 3 + 2] = tmpColor.b; }
      else { col[i * 3] = im.material.color.r; col[i * 3 + 1] = im.material.color.g; col[i * 3 + 2] = im.material.color.b; }
    }
    posMap.set(im, pos); colMap.set(im, col);
  });
...
function startMorph(toVariation) {
  ...
  morphBasePositions = current.posMap; morphBaseColors = current.colMap;
  morphFrom = currentVariation; morphTo = toVariation;
  morphStartTime = performance.now(); isMorphing = true;
}
...
function updateMorph() {
  ...
  voxels.forEach(im => {
    ...
    for (let i = 0; i < count; i++) {
      const i3 = i * 3;
      const fromX = basePos[i3], fromY = basePos[i3 + 1], fromZ = basePos[i3 + 2];
      const toX = toData.positions[i3], toY = toData.positions[i3 + 1], toZ = toData.positions[i3 + 2];
      ...
      data.origPositions[i3] = nx; data.origPositions[i3 + 1] = ny; data.origPositions[i3 + 2] = nz;
      const offX = data.offsets[i3], offY = data.offsets[i3 + 1], offZ = data.offsets[i3 + 2];
      ...
      if (im.instanceColor) {
        _morphColor.setRGB(baseCol[i3] + (toData.colors[i3] - baseCol[i3]) * localT, baseCol[i3 + 1] + (toData.colors[i3 + 1] - baseCol[i3 + 1]) * localT, baseCol[i3 + 2] + (toData.colors[i3 + 2] - baseCol[i3 + 2]) * localT);
        im.setColorAt(i, _morphColor);
      }
      needsUpdate = true;
    }
    if (needsUpdate) { im.instanceMatrix.needsUpdate = true; if (im.instanceColor) im.instanceColor.needsUpdate = true; }
  });
  if (bloomPass && bloomEnabled) {
    const fromBloom = VARIATION_BLOOM[morphFrom], toBloom = VARIATION_BLOOM[morphTo];
    const lerpedBloom = fromBloom + (toBloom - fromBloom) * t;
    bloomPass.strength.value = lerpedBloom; bloomSavedStrength = lerpedBloom;
  }
  if (t >= 1.0) { isMorphing = false; currentVariation = morphTo; morphBasePositions = null; morphBaseColors = null; }
}
...
dustPoints.name = 'dustMotes'; dustPoints.frustumCulled = false;
particleGroup.add(dustPoints);

const fallingLeafCount = 40;
const leafQuadGeo = new THREE.PlaneGeometry(0.5, 0.5);
const leafQuadMat = new THREE.MeshBasicNodeMaterial({ transparent: true, depthWrite: false, side: THREE.DoubleSide });
...
fallingLeaves.name = 'fallingLeaves'; fallingLeaves.frustumCulled = false;
particleGroup.add(fallingLeaves);

const leafState = [], leafDummy = new THREE.Object3D();
...
function resetLeaf(i) {
  const angle = Math.random() * Math.PI * 2, radius = Math.random() * 6.0;
  leafState[i] = { x: Math.cos(angle) * radius, y: 18 + Math.random() * 8, z: Math.sin(angle) * radius, vx: (Math.random() - 0.5) * 0.8, vy: -(1.5 + Math.random() * 1.5), vz: (Math.random() - 0.5) * 0.8, rotX: Math.random() * Math.PI * 2, rotY: Math.random() * Math.PI * 2, rotZ: Math.random() * Math.PI * 2, spinX: (Math.random() - 0.5) * 2.0, spinY: (Math.random() - 0.5) * 1.5, spinZ: (Math.random() - 0.5) * 2.0, scale: 0.25 + Math.random() * 0.45, wobblePhase: Math.random() * Math.PI * 2, wobbleFreq: 1.5 + Math.random() * 2.0, wobbleAmp: 0.3 + Math.random() * 0.5, life: 0, maxLife: 4 + Math.random() * 6 };
}
for (let i = 0; i < fallingLeafCount; i++) { resetLeaf(i); leafState[i].life = Math.random() * leafState[i].maxLife; }
...
function updateParticles(dt) {
  ...
  for (let i = 0; i < dustCount; i++) {
    ...
    if (dPos[i * 3] > 18) dPos[i * 3] = -18; if (dPos[i * 3] < -18) dPos[i * 3] = 18;
    if (dPos[i * 3 + 1] > 35) dPos[i * 3 + 1] = -5; if (dPos[i * 3 + 1] < -5) dPos[i * 3 + 1] = 35;
    if (dPos[i * 3 + 2] > 14) dPos[i * 3 + 2] = -14; if (dPos[i * 3 + 2] < -14) dPos[i * 3 + 2] = 14;
  }
  ...
  if (fallingLeaves._lastVariation !== currentVariation) {
    ...
    for (let j = 0; j < fallingLeafCount; j++) { const c = new THREE.Color(curPalette[Math.floor(Math.random() * curPalette.length)]); fallingLeaves.setColorAt(j, c); }
    if (fallingLeaves.instanceColor) fallingLeaves.instanceColor.needsUpdate = true;
  }
  for (let i = 0; i < fallingLeafCount; i++) {
    const s = leafState[i]; s.life += dtCl;
    if (s.life >= s.maxLife || s.y < -16) { resetLeaf(i); const c = new THREE.Color(curPalette[Math.floor(Math.random() * curPalette.length)]); fallingLeaves.setColorAt(i, c); if (fallingLeaves.instanceColor) fallingLeaves.instanceColor.needsUpdate = true; }
    const wobble = Math.sin(time * s.wobbleFreq + s.wobblePhase) * s.wobbleAmp;
    s.x += (s.vx + wobble) * dtCl; s.y += s.vy * dtCl; s.z += (s.vz + Math.cos(time * s.wobbleFreq * 0.7 + s.wobblePhase) * s.wobbleAmp * 0.6) * dtCl;
    s.rotX += s.spinX * dtCl; s.rotY += s.spinY * dtCl; s.rotZ += s.spinZ * dtCl;
    const lifeFrac = s.life / s.maxLife;
    const alpha = lifeFrac < 0.1 ? lifeFrac / 0.1 : lifeFrac > 0.85 ? (1 - lifeFrac) / 0.15 : 1.0;
    leafDummy.position.set(s.x, s.y, s.z); leafDummy.rotation.set(s.rotX, s.rotY, s.rotZ); leafDummy.scale.setScalar(s.scale * alpha); leafDummy.updateMatrix();
    fallingLeaves.setMatrixAt(i, leafDummy.matrix);
  }
  fallingLeaves.instanceMatrix.needsUpdate = true;
  if (!fallingLeaves.instanceColor) { for (let i = 0; i < fallingLeafCount; i++) { const c = new THREE.Color(curPalette[Math.floor(Math.random() * curPalette.length)]); fallingLeaves.setColorAt(i, c); } if (fallingLeaves.instanceColor) fallingLeaves.instanceColor.needsUpdate = true; }
}

// === MOUSE REPULSION ===
const raycaster = new THREE.Raycaster();
const mouse = new THREE.Vector2(9999, 9999);
const repulsionRadius = 10.0, repulsionStrength = 18.0, returnSpeed = 2.5;
...
function updateRepulsion(dt) {
  ...
  if (validHit) {
    ...
    if (!_hasSmoothedHit) { _smoothHitPoint.copy(_hitPoint); _hasSmoothedHit = true; }
    else if (!mouseIdle) { const smoothFactor = 1.0 - Math.exp(-hitSmoothSpeed * Math.min(dt, 0.05)); _smoothHitPoint.lerp(_hitPoint, smoothFactor); }
  } else { _hasSmoothedHit = false; }
  hasHit = validHit;
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (hasHit) {
        ...
        if (dist < repulsionRadius && dist > 0.01) {
          ...
          const rx = randDirs[i * 3], ry = randDirs[i * 3 + 1], rz = randDirs[i * 3 + 2];
          const radialMix = 0.6;
      ...
      const curX = offsets[i * 3], curY = offsets[i * 3 + 1], curZ = offsets[i * 3 + 2];
      ...
      if (Math.abs(newX - curX) > 0.0001 || Math.abs(newY - curY) > 0.0001 || Math.abs(newZ - curZ) > 0.0001 || Math.abs(curX) > 0.0001 || Math.abs(curY) > 0.0001 || Math.abs(curZ) > 0.0001) {
        offsets[i * 3] = newX; offsets[i * 3 + 1] = newY; offsets[i * 3 + 2] = newZ;
        im.getMatrixAt(i, _mat4); _mat4.decompose(_dummy.position, _dummy.quaternion, _dummy.scale);
        _dummy.position.set(ox + newX, oy + newY, oz + newZ); _dummy.updateMatrix(); im.setMatrixAt(i, _dummy.matrix);
        needsUpdate = true;
      }
    }
...
mainLight.shadow.camera.left = -32; mainLight.shadow.camera.right = 32;
mainLight.shadow.camera.top = 32; mainLight.shadow.camera.bottom = -32;
mainLight.shadow.bias = 0.0001; mainLight.shadow.normalBias = 0.05;
mainLight.shadow.radius = 5.0; mainLight.shadow.blurSamples = 16;
scene.add(mainLight);
...
softShadowLight.shadow.mapSize.width = 512; softShadowLight.shadow.mapSize.height = 512;
softShadowLight.shadow.camera.near = 0.5; softShadowLight.shadow.camera.far = 30;
softShadowLight.shadow.camera.left = -24; softShadowLight.shadow.camera.right = 24;
softShadowLight.shadow.camera.top = 24; softShadowLight.shadow.camera.bottom = -24;
softShadowLight.shadow.bias = 0.0001; softShadowLight.shadow.normalBias = 0.05;
softShadowLight.shadow.radius = 3.75; softShadowLight.shadow.blurSamples = 16;
scene.add(softShadowLight);
const fillLight = new THREE.DirectionalLight(0x88bbff, 1.0);
fillLight.position.set(-5, 8, -3); scene.add(fillLight);
const rimLight = new THREE.PointLight(0xffaa66, 1.5, 30);
rimLight.position.set(-4, 12, -5); scene.add(rimLight);
const accentLight = new THREE.PointLight(0xff8844, 1.2, 25);
accentLight.position.set(4, 10, 4); scene.add(accentLight);

const controls = new OrbitControls(camera, renderer.domElement);
controls.enableZoom = false;
controls.enableDamping = true;
controls.dampingFactor = 0.05;
controls.minDistance = 2; controls.maxDistance = 500;
controls.target.set(0, 6, 0);

// Post-processing
let postProcessing = null, aoPass = null, ssrPass = null, bloomPass = null;
try {
  ...
  if (ao) {
    try {
      ...
      aoPass.resolutionScale = 1.0; aoPass.samples.value = 16; aoPass.radius.value = 0.60; aoPass.distanceExponent.value = 1.0; aoPass.thickness.value = 1.60;
      const aoTexture = aoPass.getTextureNode();
      if (denoise) {
        try { const denoisePass = denoise(aoTexture, scenePassDepth, scenePassNormal, camera); denoisePass.sigma.value = 30.0; denoisePass.kSigma.value = 5.0; denoisePass.threshold.value = 0.05; currentOutput = vec3(currentOutput.rgb.mul(denoisePass.r)).toVec4(currentOutput.a); }
        catch (de) { currentOutput = vec3(currentOutput.rgb.mul(aoTexture.r)).toVec4(currentOutput.a); }
      } else { currentOutput = vec3(currentOutput.rgb.mul(aoTexture.r)).toVec4(currentOutput.a); }
    } catch (e) { console.warn('GTAO setup failed:', e); }
  }
  if (ssrModule && ssrModule.ssr) {
    try {
      ...
      ssrPass.resolutionScale = 0.25; ssrPass.thickness.value = 0.2; ssrPass.maxDistance.value = 4.0; ssrPass.samples = 4;
      const ssrStrengthUniform = uniform(0.25); window._ssrStrength = ssrStrengthUniform;
      currentOutput = vec3(currentOutput.rgb.add(ssrPass.getTextureNode().rgb.mul(ssrStrengthUniform))).toVec4(currentOutput.a);
    } catch (e) { console.warn('SSR setup failed:', e); }
  }
  const bloomFn = bloomModule && bloomModule.bloom;
  if (bloomFn) {
    try { bloomPass = bloomFn(currentOutput, 0.5, 0.2, 0.8); currentOutput = currentOutput.add(bloomPass); }
    catch (e) { console.warn('Bloom setup failed:', e); }
  }
  postProcessing.outputNode = currentOutput;
} catch (e) { console.warn('Post-processing setup failed entirely:', e); postProcessing = null; }

const aoBaseSettings = { samples: 16, radius: 0.60 };
const AO_NEAR_DIST = 3, AO_FAR_DIST = 8;
let lastAdaptiveDist = -1;
let aoEnabled = true, ssrEnabled = true, bloomEnabled = true;
let aoSavedThickness = 1.60, ssrSavedStrength = 0.25, bloomSavedStrength = 0.5;
...
function updateAdaptiveQuality() {
  ...
  const s = t * t * (3 - 2 * t);
  if (aoPass && aoEnabled) { aoPass.samples.value = Math.round(4 + (aoBaseSettings.samples - 4) * s); aoPass.radius.value = aoBaseSettings.radius * (1 + (1 - s) * 0.3); aoPass.resolutionScale = 0.75 + 0.25 * s; }
  if (ssrPass && ssrEnabled) ssrPass.resolutionScale = 0.15 + 0.10 * s;
  renderer.setPixelRatio(Math.max(1, Math.min(window.devicePixelRatio, 1.5) * (0.75 + 0.25 * s)));
}
...
function animate() {
  ...
  frameCount++;
  const now = performance.now();
  if (now - lastFpsTime >= 500) { if (fpsEl) fpsEl.textContent = Math.round(frameCount / ((now - lastFpsTime) / 1000)) + ' FPS'; frameCount = 0; lastFpsTime = now; }
  controls.update();
...
const letterColors = ["#ff6b6b","#4ecdc4","#ffe66d","#1a535c","#ff9f1c","#8338ec","#3a86ff"];
function splitLetters() {
  title.innerHTML = title.textContent.split("").map(char => char === " " ? " " : `<span class="letter">${char}</span>`).join("");
}
splitLetters();
document.querySelectorAll(".letter").forEach(letter => {
  let timeout;
  letter.addEventListener("mouseenter", () => { clearTimeout(timeout); letter.style.color = letterColors[Math.floor(Math.random() * letterColors.length)]; });
  letter.addEventListener("mouseleave", () => { timeout = setTimeout(() => { letter.style.color = ""; }, 1400); });
});
...
document.querySelector(".lang-btn").addEventListener("click", () => { menu.classList.toggle("active"); arrow.classList.toggle("rotate"); });
document.querySelectorAll(".lang-menu img").forEach(flag => {
  flag.addEventListener("click", () => { currentFlag.setAttribute("src", flag.getAttribute("src")); menu.classList.remove("active"); arrow.classList.remove("rotate"); });
});